...
<template>
  <div class="statistic">
    ...
    <Row style="padding-top: 10px" :gutter="10">
      <Col span="12">
        <div class="card">
          <div class="title">
            <p>
              <span
                :class="{ active: totalUsdt.tardeType.active === index }"
                @click="switchType(index, totalUsdt)"
                v-for="(trade, index) in totalUsdt.tardeType.type"
                :key="index"
                >{{ trade.type }}</span
              >
              <span class="detail" @click="skipRouter(totalUsdt)">明细</span>
            </p>
          </div>
          <div class="body">
            <Row>
              <Col span="7" offset="1">总数量</Col>
              <Col span="7" offset="1">热数量</Col>
              <Col span="7" offset="1">冷数量</Col>
            </Row>
            <Row style="padding-top: 10px">
              <Col span="7" offset="1">{{ totalUsdt.info.totalNum }}</Col>
              <Col span="7" offset="1">{{ totalUsdt.info.hotNum }}</Col>
              <Col span="7" offset="1">{{ totalUsdt.info.coldNum }}</Col>
            </Row>
            <Row style="padding-top: 20px">
              <Col span="7" offset="1">今日充币</Col>
              <Col span="7" offset="1">今日提币</Col>
              <Col span="7" offset="1">今日冷备份</Col>
            </Row>
            <Row style="padding-top: 10px">
              <Col span="7" offset="1">{{ totalUsdt.info.todayDeposit }}</Col>
              <Col span="7" offset="1">{{ totalUsdt.info.todayWithdraw }}</Col>
              <Col span="7" offset="1">{{ totalUsdt.info.todayCold }}</Col>
            </Row>
          </div>
          <div class="footer">
            <p></p>
      ...
      <Col span="12">
        <div class="card">
          <div class="title">
            <p>
              <span
                :class="{ active: totalMoney.tardeType.active === index }"
                @click="switchType(index, totalMoney)"
                v-for="(trade, index) in totalMoney.tardeType.type"
                :key="index"
                >{{ trade.type }}</span
              >
              <span class="detail" @click="skipRouter(totalMoney)">明细</span>
            </p>
          </div>
          <div class="body">
            <Row> <Col span="24">数量</Col> </Row>
            <Row style="padding-top: 10px">
              <Col span="24">{{ totalMoney.info.money }}</Col>
            </Row>
          </div>
          <div class="footer">
            <p>
              <span
                :class="{ active: totalMoney.timeRange.active === index }"
                v-for="(time, index) in totalMoney.timeRange.time"
                @click="selectTimeRange(index, totalMoney)"
                :key="index"
                >{{ time }}</span
              >
            </p>
      ...
      <Col span="8">
        <div class="card">
          <div class="title">
            <p>
              <span
                :class="{ active: totalVolume.tardeType.active === index }"
          ...
          <div class="body">
            ...
            <Row v-if="!!totalVolume.tabelArr.length">
              <Row
                class="info"
                v-for="tabelInfo in totalVolume.tabelArr"
                :key="tabelInfo.unit"
              >
          ...
          <div class="footer">
            <p>
              <span
                :class="{ active: totalVolume.timeRange.active === index }"
      ...
      <Col span="8">
        <div class="card">
          <div class="title">
            <p>
              <span
                :class="{ active: totalFee.tardeType.active === index }"
          ...
          <div class="body">
            ...
            <Row v-if="!!totalFee.tabelArr.length">
              <Row
                class="info"
                v-for="tabelInfo in totalFee.tabelArr"
                :key="tabelInfo.unit"
              >
          ...
          <div class="footer">
            <p>
              <span
                :class="{ active: totalFee.timeRange.active === index }"
      ...
      <Col span="8">
        <div class="card">
          <div class="title">
            <p>
              <span
                :class="{ active: totalCoin.tardeType.active === index }"
          ...
          <div class="body">
            ...
            <Row v-if="!!totalCoin.tabelArr.length">
              <Row
                class="info"
                v-for="tabelInfo in totalCoin.tabelArr"
                :key="tabelInfo.unit"
              >
                <Col span="12">{{ tabelInfo.unit }}</Col>
          ...
          <div class="footer">
            <p>
              <span
                :class="{ active: totalCoin.timeRange.active === index }"
...
import {
  getUstdStatistics,
  getDepositAndWithdrawStatistics
...
export default {
  ...
  data() {
    return {
      totalUsdt: {
        ...
        tabelArr: [],
        timeRange: { active: 0, time: ["今天", "昨天", "一月"] },
        tardeType: {
          active: 0,
          type: [
            { status: ["USDT"], type: "USDT", router: "/finance/otcdetail" }
          ]
        },
        info: {
          coldNum: 0,
      ...
      totalMoney: {
        ...
        tardeType: {
          active: 0,
          type: [
            { status: 0, type: "充值", router: "/finance/otcdetail" },
            { status: 1, type: "提现", router: "/finance/otcdetail" }
          ]
        },
        info: { money: 0 }
      },
      totalVolume: {
        ...
        tardeType: {
          ...
          type: [
            {
              status: ["OTC_NUM", "OTC_MONEY"],
              type: "法币交易",
              router: "/finance/otcdetail"
            },
            {
              status: ["EXCHANGE_BASE", "EXCHANGE_COIN"],
              type: "币币交易",
              router: "/"
            }
          ]
  ...
  methods: {
    ...
    selectTimeRange(day, obj) {
      ...
      else if (obj.type === "money") this.moneyInfo();
    },
    formatTime(day) {
            ...
            res.data.forEach(item => {
              ...
              if (saveIndex === -1) {
                if (item.type === "OTC_MONEY") {
                  this.totalVolume.tabelArr.push({
                    unit: item.unit,
                    money: item.amount
                  });
    ...
    moneyInfo() {
      getDepositAndWithdrawStatistics({
        type: this.totalMoney.tardeType.active,
        timeType: this.totalMoney.timeRange.active
      })
        .then(res => {
          if (res.code === 0) {
            this.totalMoney.info = res.data;
          } else this.$Message.error(res.message);
        })
        .catch(err => console.log(err));
    }
