...
<template>
  <div class="smeditor" id="smeditor">
    <div class="buttons" :class="buttonsBarFixed == true ? 'isFixed' :''">
      ...
      <button v-if="false" type="button" class='insert-ol' @click='insertList("OrderedList")' v-on:mouseover.stop='mouseover($event)' title="有序列表">
        <img :src='icons.listOrdered'></img>
      </button>
      <button v-if="false" type="button" class='insert-ul' @click='insertList("UnorderedList")' v-on:mouseover.stop='mouseover($event)' title="无序列表">
      ...
      document.querySelectorAll(".unchecked-list").forEach(ul => {
        ul.childNodes.forEach(li => {
          li.onclick = function(event) {
            const name =
              event.target.className === "unchecked" ? "checked" : "unchecked";
